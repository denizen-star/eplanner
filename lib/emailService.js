 ...
 * - SMTP_USERNAME (optional; SMTP auth user if different than From address)
...
class EmailService {
  ...
  _loadConfig(configPath) {
    ...
    if (process.env.EMAIL_ENABLED || process.env.SMTP_SERVER) {
      ...
      const smtpUsername = process.env.SMTP_USERNAME || senderEmail; // Backwards compatible default
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        const smtpUsername = emailConfig.smtp_username || senderEmail; // Backwards compatible default
  ...
  isEnabled() {
    ...
    const isEnabled = !!(
      this.enabled &&
      this.config.smtpServer &&
      this.config.senderEmail &&
      this.config.smtpUsername &&
      this.config.senderPassword
    );
    ...
    if (!isEnabled) {
      ...
      if (!this.config.smtpUsername) missingVars.push('SMTP_USERNAME (or missing SENDER_EMAIL fallback)');
      ...
      console.log('[EMAIL SERVICE] Email disabled. Status:', {
        ...
        hasSmtpUsername: !!this.config.smtpUsername,
  ...
  async sendEmail(options) {
    ...
    try {
      ...
      const transporter = nodemailer.createTransport({
        ...
        auth: {
          user: this.config.smtpUsername || this.config.senderEmail,
