...
class EmailService {
   ...
   * @param {string} [options.fromEmail] - Override "From" address (optional; SMTP auth still uses config)
  ...
  async sendEmail(options) {
    ...
    const { to, subject, html, text, cc, bcc, fromName, fromEmail } = options;
    ...
    try {
      ...
      const fromAddr = (fromEmail && fromEmail.trim()) ? fromEmail.trim() : this.config.senderEmail;
      ...
      if (error.response) {
        console.error('[EMAIL SERVICE] Response:', typeof error.response === 'object' ? JSON.stringify(error.response) : error.response);
      }
      if (error.stack) {
