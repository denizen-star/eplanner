...
const runs = {
  async create(runData) {
    const query = `
      INSERT INTO ep_events (
        id, uuid, location, coordinates, planner_name, title, date_time, max_participants, status, created_at,
        house_number, road, suburb, city, county, state, postcode, country, country_code,
        neighbourhood, city_district, village, town, municipality, picture, description
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  async update(runId, updates) {
    ...
    // Push runId twice for WHERE id = ? OR uuid = ?
