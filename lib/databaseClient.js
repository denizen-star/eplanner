...
const runs = {
  async create(runData) {
    const query = `
      INSERT INTO runs (id, uuid, location, coordinates, planner_name, title, date_time, max_participants, status, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
...
const signups = {
  async create(signupData) {
    const query = `
      INSERT INTO signups (run_id, name, phone, email, instagram, waiver_accepted, signed_at, metadata)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  async delete(signupId) {
    const query = `DELETE FROM signups WHERE id = ?`;
    await executeQuery(query, [signupId]);
    return true;
  },
};

// CRUD Operations for Waivers
const waivers = {
  async create(waiverData) {
    const query = `
      INSERT INTO waivers (run_id, signup_id, participant_name, participant_phone, waiver_text, timestamp, metadata)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
