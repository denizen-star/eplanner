...
const EVENT_SELECT_FIELDS = `
  ...
  event_instagram as eventInstagram,
  external_signup_enabled as externalSignupEnabled,
  payment_info_enabled as paymentInfoEnabled,
  payment_mode as paymentMode,
  total_event_cost as totalEventCost,
  payment_due_date as paymentDueDate,
  collection_locked as collectionLocked,
  collection_locked_at as collectionLockedAt
`;
...
const runs = {
  async create(runData) {
    const query = `
      INSERT INTO ep_events (
        id, uuid, location, coordinates, planner_name, title, date_time, end_time, timezone, max_participants, status, is_public, app_name, tenant_key, created_at,
        ...
        signup_link, manage_link, event_view_link, event_website, event_instagram, external_signup_enabled,
        payment_info_enabled, payment_mode, total_event_cost, payment_due_date, collection_locked, collection_locked_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    const tenantKey = runData.tenantKey || (runData.appName === 'to-lgbtq' ? 'lgbtq-hub:to' : 'eplanner:default');
    const paymentDueDate = runData.paymentDueDate ? String(runData.paymentDueDate).split('T')[0] : null;
    ...
    await executeQuery(query, [
      ...
      runData.isPublic !== undefined ? (runData.isPublic ? 1 : 0) : 1,
      runData.appName || 'eplanner',
      tenantKey,
      createdAt,
      ...
      runData.eventWebsite || null,
      runData.eventInstagram || null,
      runData.externalSignupEnabled ? 1 : 0,
      runData.paymentInfoEnabled ? 1 : 0,
      runData.paymentMode || null,
      runData.totalEventCost != null ? parseFloat(runData.totalEventCost) : null,
      paymentDueDate,
      0,
      null,
    ]);
  ...
  async getById(runId) {
    ...
    const run = processRunData(result.rows[0]);
    run.signupCount = result.rows[0].signupCount != null ? parseInt(result.rows[0].signupCount, 10) : 0;
    run.paymentInfoEnabled = Boolean(run.paymentInfoEnabled);
    run.collectionLocked = Boolean(run.collectionLocked);
    return run;
  },
  ...
  async getPublicEvents(startDate, endDate, appName = null, tenantKey = null) {
    ...
    try {
      ...
      if (error.message && (error.message.includes('is_public') || error.message.includes('app_name') || error.message.includes('tenant_key') || error.message.includes('Unknown column'))) {
        console.warn('[DATABASE] Column missing - using fallback query. Please run migrations: migration-add-app-name.sql, migration-add-tenant-key.sql');
        query = `
    ...
    return result.rows.map(run => {
      ...
      if (processed.isPublic === undefined) processed.isPublic = true;
      if (processed.appName === undefined) processed.appName = 'eplanner';
      return processed;
    });
  },
  ...
  async update(runId, updates) {
    ...
    if (updates.eventWebsite !== undefined) {
      ...
      values.push(updates.eventWebsite ? updates.eventWebsite.trim() : null);
    }
    if (updates.eventInstagram !== undefined) {
      fields.push('event_instagram = ?');
      values.push(updates.eventInstagram ? updates.eventInstagram.trim() : null);
    }
    if (updates.externalSignupEnabled !== undefined) {
      fields.push('external_signup_enabled = ?');
      values.push(updates.externalSignupEnabled ? 1 : 0);
    }
    if (updates.paymentInfoEnabled !== undefined) {
      fields.push('payment_info_enabled = ?');
      values.push(updates.paymentInfoEnabled ? 1 : 0);
    }
    if (updates.paymentMode !== undefined) {
      fields.push('payment_mode = ?');
      values.push(updates.paymentMode || null);
    }
    if (updates.totalEventCost !== undefined) {
      fields.push('total_event_cost = ?');
      values.push(updates.totalEventCost != null ? parseFloat(updates.totalEventCost) : null);
    }
    if (updates.paymentDueDate !== undefined) {
      const pd = updates.paymentDueDate ? String(updates.paymentDueDate).split('T')[0] : null;
      fields.push('payment_due_date = ?');
      values.push(pd);
    }
    if (updates.collectionLocked !== undefined) {
      fields.push('collection_locked = ?');
      values.push(updates.collectionLocked ? 1 : 0);
    }
    if (updates.collectionLockedAt !== undefined) {
      fields.push('collection_locked_at = ?');
      values.push(updates.collectionLockedAt ? toMySQLDateTime(updates.collectionLockedAt) : null);
    }
  ...
  async lockCollection(runId) {
    const run = await this.getById(runId);
    if (!run) return null;
    if (!run.paymentInfoEnabled || run.paymentMode !== 'split_cost') {
      throw new Error('Lock collection only applies to paid split-cost events');
    }
    if (run.collectionLocked) {
      throw new Error('Collection is already locked');
    }
    const signupCount = run.signupCount || 0;
    if (signupCount <= 0) {
      throw new Error('Cannot lock collection with no signups');
    }
    const total = parseFloat(run.totalEventCost);
    if (!total || total <= 0) {
      throw new Error('Invalid total event cost');
    }
    const amountPerPerson = Math.ceil((total / signupCount) * 100) / 100;
    await executeQuery(
      'UPDATE ep_signups SET amount_due = ? WHERE run_id = ?',
      [amountPerPerson, runId]
    );
    return await this.update(runId, {
      collectionLocked: true,
      collectionLockedAt: new Date().toISOString(),
    });
  },
...
const signups = {
  async create(signupData) {
    const query = `
      INSERT INTO ep_signups (run_id, name, phone, email, instagram, waiver_accepted, external_signup, signed_at, metadata, app_member_id, session_id, amount_due)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    const externalSignup = signupData.externalSignup === true ? 1 : 0;
    ...
    const result = await executeQuery(query, [
      ...
      signedAt,
      metadata,
      signupData.appMemberId ?? null,
      signupData.sessionId ?? null,
      signupData.amountDue != null ? parseFloat(signupData.amountDue) : null,
    ]);
  ...
  async update(signupId, updates) {
    const setClauses = [];
    const params = [];
    if (updates.appMemberId !== undefined) {
      setClauses.push('app_member_id = ?');
      params.push(updates.appMemberId);
    }
    if (!setClauses.length) return;
    params.push(signupId);
    await executeQuery(`UPDATE ep_signups SET ${setClauses.join(', ')} WHERE id = ?`, params);
  },
  ...
  async getByRunId(runId) {
    const query = `
      SELECT 
        ...
        external_signup as externalSignup,
        signed_at as signedAt,
        metadata,
        app_member_id as appMemberId,
    ...
    return result.rows.map(signup => {
      ...
      signup.externalSignup = Boolean(signup.externalSignup);
      return signup;
    });
  },
...
// CRUD Operations for App Members (app_members table - cross-app newsletter/member subscriptions)
const appMembers = {
  /**
   * Find all app_members rows for an identity (by email or session_id). Used to avoid duplicates and to update existing NA rows.
   * @param {string} appName
   * @param {string|null} tenantKey
   * @param {{ email?: string|null, sessionId?: string|null }} identity
   * @returns {Promise<Array<{ id, appName, tenantKey, email, name, phone, sessionId, type, status, createdAt, updatedAt }>>}
   */
  async findByIdentity(appName, tenantKey, { email, sessionId }) {
    const hasEmail = email != null && String(email).trim() !== '';
    const hasSession = sessionId != null && String(sessionId).trim() !== '';
    if (!hasEmail && !hasSession) return [];
    const params = [appName, tenantKey ?? null, hasEmail ? String(email).trim() : null, hasSession ? String(sessionId) : null];
    const query = `
      SELECT id, app_name as appName, tenant_key as tenantKey, email, name, phone, session_id as sessionId,
             type, status, created_at as createdAt, updated_at as updatedAt
      FROM app_members
      WHERE app_name = ? AND (tenant_key <=> ?) AND (email <=> ? OR session_id <=> ?)
      ORDER BY type, id
    `;
  ...
  /**
   * Upsert a passive NA member (signup-intent CTA). Prefer matching by email so we keep one row per person; always update session_id to latest. Do not upgrade status.
   */
  async upsertPassiveNaMember(appName, tenantKey, { sessionId, email, name, phone }) {
    const tk = tenantKey ?? null;
    let naRow = null;
    const hasEmail = email != null && String(email).trim() !== '';
    const hasSession = sessionId != null && String(sessionId).trim() !== '';
    if (hasEmail) {
      const byEmail = await executeQuery(
        `SELECT id, app_name as appName, tenant_key as tenantKey, email, name, phone, session_id as sessionId, type, status FROM app_members WHERE app_name = ? AND (tenant_key <=> ?) AND email = ? AND type = 'NA' LIMIT 1`,
        [appName, tk, String(email).trim()]
      );
      naRow = byEmail.rows && byEmail.rows[0] ? byEmail.rows[0] : null;
    }
    if (!naRow && hasSession) {
      const bySession = await executeQuery(
        `SELECT id, app_name as appName, tenant_key as tenantKey, email, name, phone, session_id as sessionId, type, status FROM app_members WHERE app_name = ? AND (tenant_key <=> ?) AND session_id = ? AND type = 'NA' LIMIT 1`,
        [appName, tk, String(sessionId)]
      );
      naRow = bySession.rows && bySession.rows[0] ? bySession.rows[0] : null;
    }
    if (naRow) {
      const updates = [];
      const params = [];
      if (hasEmail) { updates.push('email = ?'); params.push(String(email).trim()); }
      if (name != null && name !== '') { updates.push('name = ?'); params.push(name); }
      if (phone != null && phone !== '') { updates.push('phone = ?'); params.push(phone); }
      if (hasSession) { updates.push('session_id = ?'); params.push(sessionId); }
      if (!updates.length) return naRow;
      params.push(naRow.id);
      await executeQuery(`UPDATE app_members SET ${updates.join(', ')} WHERE id = ?`, params);
      return { ...naRow, email: hasEmail ? String(email).trim() : naRow.email, name: name || naRow.name, phone: phone || naRow.phone, sessionId: hasSession ? sessionId : naRow.sessionId };
    }
    const query = `
      INSERT INTO app_members (app_name, tenant_key, email, name, phone, session_id, type, status)
      VALUES (?, ?, ?, ?, ?, ?, 'NA', 'passive')
    `;
    await executeQuery(query, [appName, tk, hasEmail ? String(email).trim() : null, name || null, phone || null, sessionId || null]);
    return { appName, tenantKey: tk, email: hasEmail ? String(email).trim() : null, name: name || null, phone: phone || null, sessionId: sessionId || null, type: 'NA', status: 'passive' };
  },

  /**
   * Upsert a Weekly member (explicit newsletter opt-in). Keeps NA row unchanged. Insert or update type='Weekly' row with status='active' (re-activates opt-out on explicit opt-in).
   */
  async upsertWeeklyMember(appName, tenantKey, payload) {
    const { email, name, phone, session_id: sessionId } = payload;
    const tk = tenantKey ?? null;
    if (!email || typeof email !== 'string' || !email.trim()) {
      throw new Error('upsertWeeklyMember requires a non-empty email');
    }
    const normalizedEmail = email.trim().toLowerCase();
    const querySelect = `
      SELECT id, email, name, phone, session_id as sessionId, type, status
      FROM app_members
      WHERE app_name = ? AND (tenant_key <=> ?) AND email = ? AND type = 'Weekly'
      LIMIT 1
    `;
    const existing = await executeQuery(querySelect, [appName, tk, normalizedEmail]);
    const row = existing.rows && existing.rows[0];
    if (row) {
      await executeQuery(
        `UPDATE app_members SET name = ?, phone = ?, session_id = ?, status = 'active', updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        [name || null, phone || null, sessionId || null, row.id]
      );
      return { id: row.id, ...row, name: name || row.name, phone: phone || row.phone, sessionId: sessionId || row.sessionId, status: 'active' };
    }
    const insertResult = await executeQuery(
      `INSERT INTO app_members (app_name, tenant_key, email, name, phone, session_id, type, status) VALUES (?, ?, ?, ?, ?, ?, 'Weekly', 'active')`,
      [appName, tk, normalizedEmail, name || null, phone || null, sessionId || null]
    );
    const id = insertResult.insertId;
    return { id, appName, tenantKey: tk, email: normalizedEmail, name: name || null, phone: phone || null, sessionId: sessionId || null, type: 'Weekly', status: 'active' };
  },

  /**
   * Get all active Weekly members (unique by email) for digest sending.
   */
  async getActiveWeeklyMembers(appName, tenantKey) {
    const query = `
      SELECT id, app_name as appName, tenant_key as tenantKey, email, name, phone, session_id as sessionId, type, status, created_at as createdAt, updated_at as updatedAt
      FROM app_members
      WHERE app_name = ? AND (tenant_key <=> ?) AND type = 'Weekly' AND status = 'active' AND email IS NOT NULL AND email != ''
      ORDER BY email
    `;
    const result = await executeQuery(query, [appName, tenantKey ?? null]);
  ...
  /**
   * Set status for all Weekly rows for this email (applies to all sessions for that email).
   */
  async setWeeklyStatusByEmail(appName, tenantKey, email, status) {
    const query = `UPDATE app_members SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE app_name = ? AND (tenant_key <=> ?) AND email = ? AND type = 'Weekly'`;
    await executeQuery(query, [status, appName, tenantKey ?? null, email]);
  },
};

function parseTenantRow(row) {
  if (!row) return null;
  if (row.configJson && typeof row.configJson === 'string') {
    try { row.configJson = JSON.parse(row.configJson); } catch (e) { row.configJson = null; }
  }
  return row;
}

const tenants = {
  async getByKey(tenantKey, includeInactive = false) {
    const q = `SELECT tenant_key as tenantKey, product, subdomain, display_name as displayName, config_json as configJson, sender_email as senderEmail, is_active as isActive, created_at as createdAt, updated_at as updatedAt FROM ep_tenants WHERE tenant_key = ? LIMIT 1`;
    const r = await executeQuery(q, [tenantKey]);
    if (!r.rows.length) return null;
    const row = r.rows[0];
    if (!includeInactive && !row.isActive) return null;
    return parseTenantRow(row);
  },
  async getAll() {
    const q = `SELECT tenant_key as tenantKey, product, subdomain, display_name as displayName, config_json as configJson, sender_email as senderEmail, is_active as isActive, created_at as createdAt, updated_at as updatedAt FROM ep_tenants ORDER BY product, subdomain`;
    const r = await executeQuery(q, []);
    return r.rows.map(parseTenantRow);
  },
  async create(data) {
    const cfg = data.configJson ? (typeof data.configJson === 'string' ? data.configJson : JSON.stringify(data.configJson)) : null;
    const q = `INSERT INTO ep_tenants (tenant_key, product, subdomain, display_name, config_json, sender_email, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)`;
    await executeQuery(q, [data.tenantKey, data.product, data.subdomain, data.displayName || null, cfg, data.senderEmail || null, data.isActive !== false ? 1 : 0]);
    return tenants.getByKey(data.tenantKey, true);
  },
  async update(tenantKey, updates) {
    const fields = [];
    const values = [];
    if (updates.product !== undefined) { fields.push('product = ?'); values.push(updates.product); }
    if (updates.subdomain !== undefined) { fields.push('subdomain = ?'); values.push(updates.subdomain); }
    if (updates.displayName !== undefined) { fields.push('display_name = ?'); values.push(updates.displayName); }
    if (updates.configJson !== undefined) { fields.push('config_json = ?'); values.push(typeof updates.configJson === 'string' ? updates.configJson : JSON.stringify(updates.configJson)); }
    if (updates.senderEmail !== undefined) { fields.push('sender_email = ?'); values.push(updates.senderEmail || null); }
    if (updates.isActive !== undefined) { fields.push('is_active = ?'); values.push(updates.isActive ? 1 : 0); }
    if (!fields.length) return tenants.getByKey(tenantKey, true);
    values.push(tenantKey);
    await executeQuery(`UPDATE ep_tenants SET ${fields.join(', ')} WHERE tenant_key = ?`, values);
    return tenants.getByKey(tenantKey, true);
  },
  async delete(tenantKey) {
    await executeQuery(`DELETE FROM ep_tenants WHERE tenant_key = ?`, [tenantKey]);
  },
};
