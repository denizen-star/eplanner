...
const runs = {
  async create(runData) {
    const query = `
      INSERT INTO ep_events (
        id, uuid, location, coordinates, planner_name, title, date_time, end_time, timezone, max_participants, status, is_public, created_at,
  ...
  async getPublicEvents(startDate, endDate) {
    ...
    let result;
    try {
      result = await executeQuery(query, [startDateTime, endDateTime]);
    } catch (error) {
      // If query fails due to missing is_public column, use fallback query
      if (error.message && (error.message.includes('is_public') || error.message.includes('Unknown column'))) {
        console.warn('[DATABASE] is_public column missing - using fallback query. Please run migration: migration-add-public-endtime-place-links.sql');
