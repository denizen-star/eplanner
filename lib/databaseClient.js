...
const signups = {
  async create(signupData) {
    const query = `
      INSERT INTO ep_signups (run_id, name, phone, email, instagram, waiver_accepted, external_signup, signed_at, metadata)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
...
const appMembers = {
  ...
  /**
   * Upsert a passive NA member (signup-intent CTA). If a row exists for this identity (email or session), update with new values; do not upgrade status. Otherwise insert type='NA', status='passive'.
   */
  async upsertPassiveNaMember(appName, tenantKey, { sessionId, email, name, phone }) {
    const tk = tenantKey ?? null;
    const existing = await this.findByIdentity(appName, tk, { email: email || null, sessionId: sessionId || null });
    const naRow = existing.find(r => r.type === 'NA');
    if (naRow) {
      const updates = [];
      const params = [];
      if (email != null && email !== '') { updates.push('email = ?'); params.push(email); }
      if (name != null && name !== '') { updates.push('name = ?'); params.push(name); }
      if (phone != null && phone !== '') { updates.push('phone = ?'); params.push(phone); }
      if (sessionId != null && sessionId !== '') { updates.push('session_id = ?'); params.push(sessionId); }
      if (!updates.length) return naRow;
      params.push(naRow.id);
      await executeQuery(`UPDATE app_members SET ${updates.join(', ')} WHERE id = ?`, params);
      return { ...naRow, email: email ?? naRow.email, name: name ?? naRow.name, phone: phone ?? naRow.phone, sessionId: sessionId ?? naRow.sessionId };
    }
    const query = `
      INSERT INTO app_members (app_name, tenant_key, email, name, phone, session_id, type, status)
      VALUES (?, ?, ?, ?, ?, ?, 'NA', 'passive')
    `;
    await executeQuery(query, [appName, tk, email || null, name || null, phone || null, sessionId || null]);
    return { appName, tenantKey: tk, email: email || null, name: name || null, phone: phone || null, sessionId: sessionId || null, type: 'NA', status: 'passive' };
  },
  ...
  async upsertWeeklyMember(appName, tenantKey, payload) {
    ...
    if (row) {
      ...
      return { ...row, name: name || row.name, phone: phone || row.phone, sessionId: sessionId || row.sessionId, status: 'active' };
    }
    await executeQuery(
    ...
    return { appName, tenantKey: tk, email: normalizedEmail, name: name || null, phone: phone || null, sessionId: sessionId || null, type: 'Weekly', status: 'active' };
  },
