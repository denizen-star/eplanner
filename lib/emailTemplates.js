...
function eventCreatedEmail(run, coordinatorEmail, signupLink, manageLink) {
  ...
  // Generate static map if coordinates available
  const staticMapUrl = generateStaticMapUrl(run.coordinates);
  
  // Event picture (base64, if available)
  const eventPicture = run.picture ? `data:image/jpeg;base64,${run.picture}` : null;
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .event-details-glass {
      ...
      background-color: rgba(255, 255, 255, 0.15); /* Fallback for email clients */
      padding: 24px;
  ...
  <div class="email-content">
    ...
    <div class="event-details-glass">
      ...
      <div class="static-map-container">
        <img src="${staticMapUrl}" alt="Event Location Map" class="static-map" onerror="this.style.display='none';" />
        ${placeName ? `<div class="location-name">${placeName}</div>` : location && location !== 'Location TBD' ? `<div class="location-name">${location}</div>` : ''}
      </div>
      ` : ''}
    </div>
    ...
    ${eventDescription ? `
    <div class="glass-box">
      <div class="link-label">Event Description:</div>
      <div style="margin-top: 8px; color: #333; line-height: 1.6; white-space: pre-wrap;">${eventDescription.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>')}</div>
    </div>
    ` : ''}
...
<body>
  ...
  <div class="content">
    ...
    <div class="link-box">
      <div class="link-label">Event Description:</div>
      <div style="margin-top: 8px; color: #333; line-height: 1.6; white-space: pre-wrap;">${eventDescription}</div>
    </div>
    ` : ''}
...
function signupNotificationEmail(run, signup, coordinatorEmail) {
  ...
  // Accent color: #6488F4 (blue to match other emails)
  const accentColor = '#6488F4';
...
<head>
  ...
  <style>
    ...
    .participant-info { background-color: #e8f4f8; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid ${accentColor}; }
    .detail-row { margin: 10px 0; }
    .detail-label { font-weight: bold; color: #555; }
    .footer { margin-top: 20px; padding-top: 20px; border-top: 1px solid #ddd; color: #666; font-size: 12px; }
  </style>
