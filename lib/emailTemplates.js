...
/**
 * Compress image for email use (target: under 102KB to avoid Gmail clipping)
 * @param {string} base64Image - Base64 encoded image string (without data URI prefix)
 * @param {number} maxSizeKB - Maximum size in KB (default: 100KB to stay under 102KB limit)
 * @returns {Promise<string>} Compressed base64 image string
 */
async function compressImageForEmail(base64Image, maxSizeKB = 100) {
  ...
  try {
    ...
    // Try progressively smaller sizes/quality until we're under the limit
    for (let attempt = 0; attempt < 5; attempt++) {
      try {
        ...
        console.warn('[EMAIL TEMPLATE] Image compression error:', error.message);
        // If compression fails, return original (will use fallback image if too large)
        return base64Image.trim();
      }
    }
    ...
    console.warn('[EMAIL TEMPLATE] Failed to compress image for email:', error.message);
...
/**
 * Generate GenZ-style coordinator contact section
 * @param {string} coordinatorEmail - Coordinator's email address
 * @param {string} warmBeige - Background color
 * @param {string} darkBorder - Text/border color
 * @returns {string} HTML for coordinator contact section
 */
function generateCoordinatorContactSection(coordinatorEmail, warmBeige = '#F7F3ED', darkBorder = '#34495E') {
  if (!coordinatorEmail) return '';
  
  return `
    <div style="background-color: ${warmBeige}; padding: 20px; margin: 20px 0; border-radius: 8px; border-left: 4px solid ${darkBorder};">
      <p style="margin: 0; font-size: 16px; color: #333; line-height: 1.6;">
        <strong style="color: ${darkBorder}; display: block; margin-bottom: 8px;">Got questions? Hit us up! 💬</strong>
        Feel free to reach out to <a href="mailto:${coordinatorEmail}" style="color: #0066cc; text-decoration: none; font-weight: 600;">${coordinatorEmail}</a> if you need anything. We're here to help! ✨
      </p>
    </div>
  `;
}
...
/**
 * Generate email footer with decorative arch, image, and GenZ-style text
 * @param {string} imagePath - Path to footer image (relative to base URL)
 * @param {string} imageAlt - Alt text for image
 * @param {string} genZText - GenZ-style footer text about kervinapps
 * @param {string} baseUrl - Base URL for images
 * @param {string} warmBeige - Background color
 * @param {string} darkBorder - Text/border color
 * @returns {string} HTML for email footer
 */
function generateEmailFooter(imagePath, imageAlt, genZText, baseUrl = 'https://eplanner.kervinapps.com', warmBeige = '#F7F3ED', darkBorder = '#34495E') {
  const imageUrl = imagePath.startsWith('http') ? imagePath : `${baseUrl}/${imagePath}`;
  
  return `
    <div style="background-color: ${warmBeige}; padding: 0; margin-top: 40px; position: relative; border-radius: 8px; border: 1px solid ${darkBorder}; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
      
      <!-- Footer Image with 30% opacity -->
      <div style="width: 100%; text-align: center; padding: 20px 0;">
        <img src="${imageUrl}" alt="${imageAlt}" style="max-width: 100%; height: auto; opacity: 0.3; display: block; margin: 0 auto;" />
      </div>
      
      <!-- GenZ Footer Text -->
      <div style="padding: 20px; text-align: center; border-top: 1px solid rgba(68, 57, 13, 0.1);">
        <p style="margin: 0; font-size: 14px; color: #666; line-height: 1.6;">
          ${genZText}
        </p>
        <p style="margin: 12px 0 0 0; font-size: 12px; color: #999;">
          Powered by <strong style="color: ${darkBorder};">kervinapps</strong> - Making community events happen, one gathering at a time! 🚀
        </p>
        <p style="margin-top: 20px; font-size: 10px; color: #999; line-height: 1.4;">
          Disclaimer: Kervinapps is not responsible or liable for any activity planned on this website. This website is solely a tool to facilitate event creation and management.
        </p>
      </div>
    </div>
  `;
 ...
 */
async function eventCreatedEmail(run, coordinatorEmail, signupLink, manageLink) {
  const eventTitle = run.title || 'Event';
  ...
  // Use data URI for email compatibility (email clients block API endpoints)
  // Compress image to stay under 102KB to avoid Gmail clipping
  let eventPicture = defaultEventImage;
  if (run.picture && typeof run.picture === 'string' && run.picture.trim()) {
    const compressedImage = await compressImageForEmail(run.picture.trim());
    if (compressedImage) {
      eventPicture = `data:image/jpeg;base64,${compressedImage}`;
    }
  }

  // Color constants
  const warmBeige = '#F7F3ED';
  const darkBorder = '#34495E';
  const tealButton = '#34495E';
...
<body>
  ...
  <div class="email-content">
    ...
    <div class="glass-box qr-code-box">
      <div class="link-label">Invite Friends:</div>
      <img src="${qrCodeUrl}" alt="QR Code for ${signupLink}" class="qr-code-img" />
      <div class="link-url" style="margin: 12px 0;"><a href="${signupLink}" style="color: #0066cc; word-break: break-all;">${signupLink}</a></div>
      <p class="link-description" style="font-size: 18px; color: #333; margin-top: 16px; font-weight: 600; line-height: 1.6;">No gatekeeping allowed—send the QR/ Link to your friends! It's just gonna be a chill time. Lets catch the vibe togeher.</p>
    </div>
  </div>
  
  ${generateCoordinatorContactSection(coordinatorEmail, warmBeige, darkBorder)}
  
  ${generateEmailFooter('assets/images/HappyConfirm.jpeg', 'Happy Confirmation', 'Thanks for being part of our community! We\'re stoked to help you bring people together. 💙', baseUrl, warmBeige, darkBorder)}
</body>
</html>
  `.trim();
 ...
 * @returns {Object} { html, text } - HTML and plain text versions
 */
async function signupConfirmationEmail(run, signup, eventViewLink = null, externalSignup = false) {
  const eventTitle = run.title || 'Event';
  const plannerName = run.plannerName || run.pacerName || 'Event Coordinator';
  const location = run.location || 'Location TBD';
  ...
  if (eventViewLink) {
    try {
      ...
      console.warn('[EMAIL TEMPLATE] Failed to extract baseUrl from eventViewLink:', eventViewLink);
    }
  }
  // Ensure baseUrl always ends without trailing slash for consistency
  baseUrl = baseUrl.replace(/\/$/, '');
  

  // Generate signup link and QR code
...
${externalSignup ? `\nNote: This event uses an external signup page and is not tracked on this website.\n` : ''}
...
<body>
  ...
  <div class="email-content">
    ...
    ${externalSignup ? '<p style="padding: 0 20px 10px 20px; font-size: 16px;"><strong>Note:</strong> This event uses an external signup page and is not tracked on this website.</p>' : ''}

    <p style="padding: 0 20px 20px 20px; font-size: 16px;">We look forward to seeing you there!</p>
    
    ${run.coordinatorEmail ? generateCoordinatorContactSection(run.coordinatorEmail, warmBeige, darkBorder) : ''}
  </div>
  
  ${generateEmailFooter('assets/images/HappyConfirm.jpeg', 'Happy Confirmation', 'Thanks for being part of our community! We\'re stoked to help you bring people together. 💙', baseUrl || 'https://eplanner.kervinapps.com', warmBeige, darkBorder)}
</body>
</html>
  `.trim();
...
<body>
  ...
  ${generateEmailFooter('assets/images/HappyConfirm.jpeg', 'New Signup', 'Thanks for being an awesome event coordinator! We\'re stoked to help you bring people together. 💙', 'https://eplanner.kervinapps.com', warmBeige, darkBorder)}
</body>
</html>
  `.trim();
...
<body>
  ...
  ${generateEmailFooter('assets/images/HappyConfirm.jpeg', 'Event Updated', 'Thanks for keeping your event details fresh! We\'re here to help make your gathering awesome. 💙', 'https://eplanner.kervinapps.com', warmBeige, darkBorder)}
</body>
</html>
  `.trim();
...
<body>
  ...
  <div class="email-content">
    ...
    <p style="padding: 0 20px 20px 20px; font-size: 16px;">Please review the updated details.</p>
    
    ${run.coordinatorEmail ? generateCoordinatorContactSection(run.coordinatorEmail, warmBeige, darkBorder) : ''}
  </div>
  
  ${generateEmailFooter('assets/images/HappyConfirm.jpeg', 'Event Updated', 'Thanks for staying in the loop! We\'re here to help make your experience awesome. 💙', 'https://eplanner.kervinapps.com', warmBeige, darkBorder)}
</body>
</html>
  `.trim();
...
/**
 * Generate event cancellation email for signups
 * @param {Object} run - Cancelled event/run object
 * @param {Object} signup - Signup object
 * @returns {Object} { html, text, subject, fromName } - HTML and plain text versions
 */
function eventCancelledEmail(run, signup, cancellationMessage = null) {
  ...
  // Generate "From" name: "<Event Title> - Cancellation" or "Event Cancellation" if no title
  const fromName = eventTitle && eventTitle !== 'Event' 
    ? `${eventTitle} - Cancellation`
    : 'Event Cancellation';
...
${cancellationMessage ? `Message from coordinator:\n${cancellationMessage}\n\n` : ''}We apologize for any inconvenience this may cause. If you have any questions, please contact ${coordinatorEmail}.

Thank you for your understanding.
  `.trim();

  const baseUrl = process.env.BASE_URL || 'https://eplanner.kervinapps.com';
  const defaultEventImage = `${baseUrl}/assets/images/emailfallback.png`;
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .cancellation-message-box strong {
      display: block;
      margin-bottom: 8px;
      color: #7f1d1d;
      font-size: 16px;
...
<body>
  <div class="email-header">
    ...
    <div class="header-title">Event Cancelled</div>
  </div>
  
  <div class="email-content">
    <div class="content-section">
      <p class="greeting">Hi ${participantName},</p>
      ...
      <div class="cancellation-message-box">
      ...
      <div class="coordinator-contact">
        <p style="margin: 0; font-size: 16px;"><strong>Questions?</strong> Feel free to reach out to ${coordinatorEmail} if you have any questions about this cancellation. We appreciate your understanding! 💙</p>
      </div>
      
      <p style="padding: 0 20px 20px 20px; font-size: 16px;">Thank you for your understanding.</p>
    </div>
  </div>
  
  ${generateEmailFooter('assets/images/Cancellation.jpeg', 'Event Cancelled', 'We\'re sorry this event was cancelled. Thanks for being part of our community, and we hope to see you at future events! 💙', baseUrl, warmBeige, darkBorder)}
</body>
</html>
  `.trim();
